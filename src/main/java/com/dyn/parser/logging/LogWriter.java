package com.dyn.parser.logging;
...
public class LogWriter {
	...
	static {
		logger = Logger.getLogger("DYN Roster Manager");
		...
		try {
			...
			File file = new File(dir, "Logs Parser-latest.log");
			File lock = new File(dir, "Logs Parser-latest.log.lck");
			File file2 = new File(dir, "Logs Parser-1.log");
			File file3 = new File(dir, "Logs Parser-2.log");
			File file4 = new File(dir, "Logs Parser-3.log");
			if (lock.exists()) {
				lock.delete();
			}
			if (file4.exists()) {
			...
			(LogWriter.handler = new StreamHandler(new FileOutputStream(file), new Formatter() {
				...
				public String format(LogRecord record) {
					...
					String time = "[" + LogWriter.dateformat.format(new Date(record.getMillis())) + "]["
							+ record.getLevel() + "/" + "Logs Parser" + "]" + line;
